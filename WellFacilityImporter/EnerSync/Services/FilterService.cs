...
namespace EnerSync.Services
{
    public class FilterService
    {
        ...
        public IAsyncEnumerable<T> GetFilteredDataAsync<T>(List<FilterCriteria> criteria) where T : class
        {
            ...
            foreach (var criterion in criteria)
            {
                var property = typeof(T).GetProperty(criterion.FieldName)
                    ?? throw new ArgumentException($"Property '{criterion.FieldName}' does not exist on type '{typeof(T).Name}'");
                var propertyAccess = Expression.Property(parameter, property);
                ...
                Expression comparison = criterion.Operator switch
                {
                    ...
                    // String-specific operators
                    "Contains" when property.PropertyType == typeof(string) =>
                        Expression.Call(propertyAccess, typeof(string).GetMethod("Contains", new[] { typeof(string) })!, typedValue),
                    "DoesNotContain" when property.PropertyType == typeof(string) =>
                        Expression.Not(Expression.Call(propertyAccess, typeof(string).GetMethod("Contains", new[] { typeof(string) })!, typedValue)),
                    "StartsWith" when property.PropertyType == typeof(string) =>
                        Expression.Call(propertyAccess, typeof(string).GetMethod("StartsWith", new[] { typeof(string) })!, typedValue),
                    "EndsWith" when property.PropertyType == typeof(string) =>
                        Expression.Call(propertyAccess, typeof(string).GetMethod("EndsWith", new[] { typeof(string) })!, typedValue),
                    "IsEmpty" when property.PropertyType == typeof(string) =>
                        Expression.Equal(propertyAccess, Expression.Constant(string.Empty)),
                    "IsNotEmpty" when property.PropertyType == typeof(string) =>
                        Expression.NotEqual(propertyAccess, Expression.Constant(string.Empty)),
                ...
                // Combine expressions using the same parameter
                if (combinedExpression == null)
                {
                ...
                else
                {
                    combinedExpression = criterion.LogicalOperator switch
                    {
                        "AND" => Expression.AndAlso(combinedExpression, comparison),
                        "OR" => Expression.OrElse(combinedExpression, comparison),
