...
public class CoordinatesToDlsConverter
{
    ...
    public static DLS ConvertCoordinatesToDls(SqlDouble latitude, SqlDouble longitude)
    {

        if (latitude.IsNull || longitude.IsNull)
        {
            return DLS.Null; // One or more input coordinates are NULL.
        }
        ...
        try
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                ...
                identifiedTownship = DetermineTownshipFromLatitude(latitude.Value);
                ...
                using (SqlCommand cmd = new SqlCommand(queryClosestSection, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            double distance = HaversineDistance(latitude.Value, longitude.Value, baseLatitude, baseLongitude);
                ...
                double deltaLatDegrees = latitude.Value - baseLatitudeSection;
                double deltaLongDegrees = longitude.Value - baseLongitudeSection;
